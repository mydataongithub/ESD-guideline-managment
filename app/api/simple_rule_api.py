...
simple_router = APIRouter(prefix="/api/simple-rules", tags=["simple-rules"])

@simple_router.post("/")
async def create_rule_simple(
    rule_data: Dict[str, Any] = Body(...),
    db: Session = Depends(get_db)
):
    """
    Create a new rule using a direct, simplified approach.
    This endpoint bypasses Pydantic models to avoid versioning issues.
    """
